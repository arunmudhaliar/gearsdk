...
zhandle_t* qzookeeper::zh = nullptr;
timeval qzookeeper::startTime;
int qzookeeper::batchMode=0;
int qzookeeper::to_send=0;
int qzookeeper::sent=0;
int qzookeeper::recvd=0;
int qzookeeper::shutdownThisThing=0;
...
void qzookeeper::my_string_completion(int rc, const char *name, const void *data) {
...
void qzookeeper::my_string_completion_free_data(int rc, const char *name, const void *data) {
    my_string_completion(rc, name, data);
    free((void*)data);
}

void qzookeeper::my_string_stat_completion(int rc, const char *name, const struct Stat *stat,
        const void *data)  {
    my_string_completion(rc, name, data);
    dumpStat(stat);
}

void qzookeeper::my_string_stat_completion_free_data(int rc, const char *name,
...
void qzookeeper::my_silent_data_completion(int rc, const char *value, int value_len,
        const struct Stat *stat, const void *data) {
    recvd++;
...
void qzookeeper::my_strings_completion(int rc, const struct String_vector *strings,
        const void *data) {
    struct timeval tv;
    int sec;
    int usec;
...
void qzookeeper::my_strings_stat_completion(int rc, const struct String_vector *strings,
        const struct Stat *stat, const void *data) {
...
void qzookeeper::my_silent_stat_completion(int rc, const struct Stat *stat,
        const void *data) {
...
void qzookeeper::sendRequest(const char* data) {
    zoo_aset(zh, "/od", data, strlen(data), -1, my_silent_stat_completion,
            strdup("/od"));
    zoo_aget(zh, "/od", 1, my_silent_data_completion, strdup("/od"));
}

void qzookeeper::od_completion(int rc, const struct Stat *stat, const void *data) {
    int i;
...
int qzookeeper::startsWith(const char *line, const char *prefix) {
    int len = strlen(prefix);
    return strncmp(line, prefix, len) == 0;
}

void qzookeeper::processline(const char *line) {
    int rc;
        ...
        if (rc) {
           ...
           while (p != NULL) {
               if (strcmp(p, "-add")==0) {
                   ...
                   members = (char*)calloc(members_size + 1, sizeof(char));
                   if(members == NULL )
                   {
        ...
        if (state == ZOO_CONNECTED_STATE) {
            ...
            if (qzk->myid.client_id == 0 || qzk->myid.client_id != id->client_id) {
                ...
                if (qzk->clientIdFile) {
                    ...
                    if (!fh) {
                        perror(qzk->clientIdFile);
                    } else {
                        int rc = fwrite(&qzk->myid, sizeof(qzk->myid), 1, fh);
 ...
 */
int qzookeeper::handleBatchMode(const char* arg, const char** buf) {
...
int qzookeeper::init(int argc, char **argv) {
...
#ifdef HAVE_OPENSSL_H
        if (!cert) {
            zh = zookeeper_init(hostPort, watcher, 30000, &myid, this, flags);
        } else {
            zh = zookeeper_init_ssl(hostPort, cert, watcher, 30000, &myid, this, flags);
        }
#else
        zh = zookeeper_init(hostPort, watcher, 30000, &myid, this, flags);
#endif
...
int qzookeeper::init_test(const qstring& url) {
    zoo_deterministic_conn_order(1); // enable deterministic order
...
#else
        zh = zookeeper_init(url.c_str(), watcher, 30000, &myid, this, flags);
#endif

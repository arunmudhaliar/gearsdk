...
const char* qzookeeper::state_to_string(int state) {
	if (state == 0)
		return "CLOSED_STATE";
...
const char* qzookeeper::type_to_string(int state) {
...
const char* qzookeeper::get_name() const {
	std::unique_lock<std::timed_mutex> lock(name_mutex, std::defer_lock);
...
const char* qzookeeper::get_wname() const {
	std::unique_lock<std::timed_mutex> lock(wname_mutex, std::defer_lock);
	if (lock.try_lock_for(std::chrono::milliseconds(100))) {
		return WNAME.c_str();
	} else {
		return __LOGTAG__;
...
void qzookeeper::watcher(zhandle_t* zzh, int type, int state, const char* path, void* context) {
	...
	debug_print(LOG_LEVEL_0, logtag, "Watcher %s state = %s", type_to_string(type), state_to_string(state));
	...
	if (type == ZOO_SESSION_EVENT) {
		if (state == ZOO_CONNECTED_STATE) {
			...
			if (qzk->myid.client_id == 0 || qzk->myid.client_id != id->client_id) {
				...
				if (qzk->client_id_file) {
					debug_print(LOG_LEVEL_0, logtag, "client_id_file %s", qzk->client_id_file);
					FILE* fh = fopen(qzk->client_id_file, "w");
					if (!fh) {
						debug_print_error(logtag, "fopen failed %s", qzk->client_id_file);
					} else {
				...
				for (auto p : qzk->pending_config_updates) {
					...
					auto operation = [&]() { return zoo_wget(zzh, p.c_str(), watcher, context, buffer, &buffer_len, nullptr); };
		...
		if (state == ZOO_CONNECTED_STATE) {
			char buffer[4 * 1024];
			int buffer_len = sizeof(buffer);
			auto operation = [&]() { return zoo_wget(zzh, path, watcher, context, buffer, &buffer_len, nullptr); };
			int rc = retry_with_backoff(operation, QZK_MAX_RETRIES_FOR_API, QZK_RETRY_BASE_DELAY_MS);
...
int qzookeeper::connect(const qstring& url) {
	...
	if (connection_state != ZOO_CONNECTED_STATE) {
		debug_print_error(logtag, "qzookeeper not in ZOO_CONNECTED_STATE state while returning from connect - %s", state_to_string(connection_state));
	}
...
int qzookeeper::retry_connection() {
	...
	if (reconnect_mutex.try_lock(__FUNCTION__) != 0) {
...
void* qzookeeper::connect_internal(void* data) {
	...
	if (thiz->reconnect_mutex.try_lock(__FUNCTION__) != 0) {
			...
			if (!(thiz->connection_state == ZOO_CONNECTING_STATE || thiz->connection_state == ZOO_ASSOCIATING_STATE || thiz->connection_state == ZOO_CONNECTED_STATE) /*&& !thiz->connection_in_progress*/) {
				...
				if (thiz->retry_count < 5) {
					debug_print_error(logtag,
									  ...
									  thiz->retry_count, qzookeeper::state_to_string(thiz->connection_state), thiz->connection_state);
					thiz->retry_connection();
				} else {
					debug_print_important2(logtag, "mainloop break %s", qzookeeper::state_to_string(thiz->connection_state));
	...
	debug_print_important2(logtag, "mainloop exited with connection state %s", qzookeeper::state_to_string(thiz->connection_state));
...
void qzookeeper::dump_stat(const struct Stat* stat) {
	char tctimes[40];
	char tmtimes[40];
	time_t tctime;
	time_t tmtime;
	...
	if (!stat) {
		debug_print_error(__LOGTAG__, "[zk] : dump_stat --> null");
		return;
	}
...
int qzookeeper::set_data(const qstring& zk_path, const qstring& data) {
	...
	if (!zh) {
		debug_print_error(__LOGTAG__, "qzookeeper zh == null - %s", state_to_string(connection_state));
		return -1;
	}
	if (connection_state != ZOO_CONNECTED_STATE) {
		debug_print_error(__LOGTAG__,
						  ...
						  zk_path.c_str(), connection_state, state_to_string(connection_state));
		return -2;
	}
	if (op_in_progress) {
		return -5;
...
void qzookeeper::close_zk(const int STATE) {
	const char* logtag = get_name();  // its safe to read since no one going to write on this variable except constructor;
	UNUSED(STATE);
	if (close_mutex.try_lock(__FUNCTION__) != 0) {

...
struct st_qh3router_config_in {
	st_qh3router_config_in() {}
	st_qh3router_config_in(const qstring& host, const qstring& port, const qstring& mongodb_uri, const qstring& redis_ip, uint16_t redis_port_, const fs::path& root_dir, const fs::path& inf_file, struct addrinfo* router,
						   uint16_t command_port, const qstring& router_port, const qstring& zk_uri, uint16_t router_port_return, const qstring& app_id, void* user_arg = nullptr)
		: host(host),
		  port(port),
	...
	st_qh3router_config_in(const qstring& host, const qstring& port, const qstring& mongodb_uri, const qstring& redis_ip, uint16_t redis_port_, const fs::path& root_dir, const fs::path& inf_file, const qstring& zk_uri,
	...
	st_qh3router_config_in(const st_qh3router_config_in& config)
		: host(config.host),
		  port(config.port),
	...
	st_qh3router_config_in& operator=(const st_qh3router_config_in& config) {
		if (this == &config)
	...
	void print() {
		debug_print(LOG_LEVEL_0, __LOGTAG__, "st_qh3router_config_in values:\n{");
...
struct st_qh3server_config_in {
	...
	st_qh3server_config_in(const st_qh3router_config_in& config)
		: host(config.host),
		  port(config.port),
		  root_dir(config.root_dir),
		  inf_file(config.inf_file),
...
struct st_qserver_config_in {
	st_qserver_config_in() {}
	st_qserver_config_in(const qstring& host, const qstring& port, const qstring& mongodb_uri, const qstring& redis_ip, uint16_t redis_port_, const fs::path& root_dir, const fs::path& inf_file, const qstring& zk_uri, const qstring& app_id,
						 void* user_arg = nullptr)
		: host(host), port(port), mongodb_uri(mongodb_uri), redis_ip(redis_ip), redis_port(redis_port_), zk_uri(zk_uri), root_dir(root_dir), inf_file(inf_file), app_id(app_id), user_arg(user_arg) {}
	st_qserver_config_in(const st_qserver_config_in& config) {
		host = config.host;
		port = config.port;
		mongodb_uri = config.mongodb_uri;
		redis_ip = config.redis_ip;
	...
	// Copy Assignment Operator
	st_qserver_config_in& operator=(const st_qserver_config_in& config) {
		if (this == &config)
			return *this;  // Prevent self-assignment
		host = config.host;
		port = config.port;
	...
	void print() {
		debug_print(LOG_LEVEL_0, __LOGTAG__, "st_qserver_config_in values:\n{");
		debug_raw(LOG_LEVEL_0, qstring::format_string("Host: %s", host.c_str()).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("Port: %s", port.c_str()).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("MongoDB URI: %s", mongodb_uri.c_str()).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("Redis IP: %s", redis_ip.c_str()).c_str());
...
struct st_services_config {
	...
	st_services_config(const st_qh3router_config_in& config) {
		redis_ip = config.redis_ip;
		redis_port = config.redis_port;
		mongodb_uri = config.mongodb_uri;
		zk_uri = config.zk_uri;
	...
	void print() {
		debug_print(LOG_LEVEL_0, __LOGTAG__, "st_services_config values:\n{");
		debug_raw(LOG_LEVEL_0, qstring::format_string("MongoDB URI: %s", mongodb_uri.c_str()).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("Redis IP: %s", redis_ip.c_str()).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("Redis Port: %u", redis_port).c_str());
		debug_raw(LOG_LEVEL_0, qstring::format_string("Zookeeper URI: %s", zk_uri.c_str()).c_str());

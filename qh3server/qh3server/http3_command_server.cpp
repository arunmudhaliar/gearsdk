...
http3_command_server::http3_command_server(const qstring& redis_ip,
										   uint16_t redis_port,
										   bridge_command_center* bridge_,
										   qstring router_port_)
...
void http3_command_server::on_run_started() {
	hiredis->set_hash_value(
		qstring::format_string("servers:%s", host_id.c_str()), "command_center",
...
void http3_command_server::parse_header(const qstring& name,
										const qstring& value,
...
void http3_command_server::parse(struct conn_io_qh3* conn_io) {
	...
	if (path_header == nullptr) {
		...
		qh3server::get_stats_loggeer()->server_count(
			"parse", 1, "", "", "", "error", "http3_command_server", "",
	...
	if (path_header->value.length() <= 1) {
		...
		qh3server::get_stats_loggeer()->server_count(
			"parse", 1, "", "", "", "warn", "http3_command_server",
...
void http3_command_server::parse_shutdown_command_center(
...
void http3_command_server::parse_shutdown_test(
	...
	conn_io->http_response->set_payload(
		qstring::format_string("{ %d-shutdown-all }", getpid()));
}

void http3_command_server::parse_whoami(conn_io_req_res::header* path_header,
										struct conn_io_qh3* conn_io) {
	const char* const_logtag = logtag.c_str();
	const char* port_id_cstr = port_id.c_str();
	bool has_crc_header = conn_io->http_request->has_crc_header();
	...
	qh3server::get_file_logger()->log(
		qlogfile::level_0, const_logtag, "%s - whoami - %s",
		path_header->value.c_str(), payload.c_str());
	qh3server::get_stats_loggeer()->server_count(
		"parse", 1, "", "", "", "command", "http3_command_server",
		has_crc_header ? "" : "no-crc", port_id_cstr, path_header->value.c_str());
}
...
void http3_command_server::construct_response_whoami(qstring& response_string) {
	...
	const qstring& server_id =
		qstring::format_string("%d-http3_command_server", getpid());
	doc.AddMember("name", Value().SetString(server_id.c_str(), allocator),
				  allocator);
	Value servers(kArrayType);

	hiredis->iterate_hash(
		qstring::format_string("servers:%s", host_id.c_str()), &doc,
		[&allocator, &servers](const char* field, const char* value, void* arg) {
			UNUSED(arg);
	...
	hiredis->iterate_hash(
		"gservers", &doc,
		[&allocator, &gservers](const char* field, const char* value, void* arg) {
			UNUSED(arg);
...
void http3_command_server::send_shutdown_to_all() {
	...
	qh3client_helper::send_async_request<client::qh3client>(
		...
		[](conn_io_req_res* response, void* client_specific_data, void* arg,
		   bool success) {
			UNUSED(response);
...
void http3_command_server::command_feedback_recv_cb(EV_P_ ev_io* w,
													int revents) {
	UNUSED(loop);
	UNUSED(revents);
	...
	while (1) {
		...
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__,
					"RECEIVED cmd feedback from client - %s !!!", cmd.c_str());
		bridge->cmd_feedback_from_client((struct sockaddr*) &peer_addr,
										 cmd.c_str());
	}
}

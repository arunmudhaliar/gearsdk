...
qh3simple_router::qh3simple_router(const server_config_in& config)
	: config(config) {}
...
int qh3simple_router::run() {
	...
	freeaddrinfo(router);
	if (getaddrinfo(config.host.c_str(), config.port.c_str(), &hints, &router) !=
		0) {
	...
	if (getaddrinfo(
			config.host.c_str(),
			qstring::format_string("%d", config.router_port_return).c_str(),
			&hints, &router_return) != 0) {
	...
	while (spawned_servers < 5) {
		...
		int result =
			spawn_qh3server(config.host, qstring::format_string("%d", free_port),
							config, child_process_id, fork_result);
#if FORK_QH3_SERVER
		UNUSED(result);
		if (fork_result) { // forking is successfull
			...
			if (parent_process_id == getpid()) {
				...
				DEBUG_PRINT_ERROR(
					__LOGTAG__,
					"forking failed, allowing parent(pid:%d) to continue !!!",
					parent_process_id);
				continue;
			}
		}
	...
	if (child_process_id != 0) {
		...
		if (bind(sock_return, router_return->ai_addr, router_return->ai_addrlen) <
			0) {
		...
		hiredis->set_hash_value(
			qstring::format_string("servers:%s", config.host.c_str()), "router",
			qstring::format_string("%s:%s", config.host.c_str(),
								   config.port.c_str()));
		hiredis->set_hash_value(
			qstring::format_string("servers:%s", config.host.c_str()),
			"router-return",
			qstring::format_string("%s:%d", config.host.c_str(),
		...
		// command server
		if (spawn_qh3server_command_server(config.host, config.command_port,
										   config) == nullptr) {
...
void qh3simple_router::recv_cb(EV_P_ ev_io* w, int revents) {
	...
	while (1) {
		...
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__, "from client %s:%s read:%d", name,
					port, read);
#endif
		...
		//        dcid_len);
		crc_ = essentials::mod_crc32_z(crc_, (const unsigned char*) &buf[read],
									   peer_addr_len);
...
void qh3simple_router::recv_return_cb(EV_P_ ev_io* w, int revents) {
	...
	while (1) {
		...
		ssize_t read = recvfrom(router->sock_return, buf_return, sizeof(buf_return),
		...
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__,
					"recv_return - from server %s:%s read:%d", name, port, read);
#endif
		...
		memcpy(&client_info->sa_data[2], &buf_return[read + 2],
			   4); // 0.0.0.0 = 4 bytes
		EV_STOP_RECORD(router_server_port_deserialise_time, __LOGTAG__,
					   "router_server_port_deserialise_time t:%lu ms", 10);
		//

		ssize_t sent =
			sendto(router->sock, buf_return, read, 0, client_info, peer_addr_len);
#if LOG_LEVEL >= LOG_LEVEL_4
		getnameinfo((struct sockaddr*) &peer_addr, sizeof(struct sockaddr), name,
					sizeof(name), port, sizeof(port),
					NI_NUMERICHOST | NI_NUMERICSERV);
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__, "recv_return - send to %s:%s bytes:%d",
					name, port, sent);
#endif
		...
		if (sent != read) {
			...
			getnameinfo((struct sockaddr*) &peer_addr, sizeof(struct sockaddr), name,
						sizeof(name), port, sizeof(port),
						NI_NUMERICHOST | NI_NUMERICSERV);
			DEBUG_PRINT_ERROR(__LOGTAG__, "ERROR recv_return - sending to %s:%s",
							  name, port);
			DEBUG_PRINT_ERROR(__LOGTAG__, "recv_return - failed to send %d<>%d", sent,
							  read);
		}
	}
}

route* qh3simple_router::spawn_qh3server_command_server(
	const qstring& host, const qstring& port, const server_config_in& config) {
	server_config_in* new_config = DEBUG_NEW server_config_in(
		host, port, config.mongodb_uri, config.redis_ip, config.redis_port,
		config.rootDir, nullptr, config.command_port, config.router_port,
		config.zk_uri, config.router_port_return);
	new_config->command_server = true;
	new_config->ref = this;

	if (pthread_create(&new_config->run_thread_id, nullptr,
					   qh3simple_router::spawn_qh3server_internal,
					   (void*) new_config) < 0) {
		DEBUG_PRINT_ERROR(
			__LOGTAG__,
			"spawn_qh3server_command_server - could not create thread: %s - %d",
	...
	GX_DELETE(command_feedback_route);
	command_feedback_route = DEBUG_NEW route(
		host, qstring::format_string("%d", new_config->command_feedback_port),
		-1);
	command_feedback_route->create_bridge(
		mainloop, (bridge_command_center*) this,
		http3_command_server::command_feedback_recv_cb);

	GX_DELETE(command_route);
	command_route = DEBUG_NEW route(host, port, server_counter++);
	DEBUG_PRINT_IMPORTANT2(__LOGTAG__, "spawned qh3 command server: %s:%s id-%d",
						   host.c_str(), port.c_str(), command_route->server_id);
...
int qh3simple_router::spawn_qh3server(const qstring& host, const qstring& port,
	...
	if (child_process_id == 0) {
		...
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__, "Child process (PID: %d) [%d]",
					getpid(), child_process_id);
		server_config_in* new_config = DEBUG_NEW server_config_in(
			host, port, config.mongodb_uri, config.redis_ip, config.redis_port,
			config.rootDir, router, config.command_port, config.router_port,
			config.zk_uri, config.router_port_return);
		if (pthread_create(&new_config->run_thread_id, nullptr,
						   qh3simple_router::spawn_qh3server_internal,
						   (void*) new_config) < 0) {
			DEBUG_PRINT_ERROR(__LOGTAG__,
							  "spawn_qh3server - could not create thread: %s - %d",
							  strerror(errno), errno);
			GX_DELETE(new_config);
			return -1;
		}
		...
		if (pthread_join(new_config->run_thread_id, nullptr) != 0) {
			DEBUG_PRINT_ERROR(__LOGTAG__,
							  "spawn_qh3server - could not join thread: %s - %d",
							  strerror(errno), errno);
		}
		...
		DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__,
					"Parent process after fork (PID: %d) [%d]", getpid(),
					child_process_id);
		route* child = DEBUG_NEW route(host, port, server_counter++);
		child->child_process_id = child_process_id;
		routes.push_back(child);
		DEBUG_PRINT_IMPORTANT2(__LOGTAG__, "spawned qh3server: %s:%s id-%d",
							   host.c_str(), port.c_str(), child->server_id);
...
void* qh3simple_router::spawn_qh3server_internal(void* data) {
	...
	if (config->command_server) {
		...
		http3_command_server* new_server = DEBUG_NEW http3_command_server(
			redis_ip.c_str(), redis_port, config->ref, config->router_port);
		new_server->run(host.c_str(), port.c_str(), rootDir, config->router,
						config->command_feedback_port, config->router_port_return);
		GX_DELETE(new_server);
	} else {
		PTHREAD_NAME("http3_sample_server");
		http3_sample_server* new_server = DEBUG_NEW http3_sample_server(
			mongodb_uri.c_str(), redis_ip.c_str(), redis_port, zk_uri);
		new_server->run(host.c_str(), port.c_str(), rootDir, config->router,
						config->command_feedback_port, config->router_port_return);
		GX_DELETE(new_server);
	}
...
int qh3simple_router::next_available_port(const qstring& host,
										  port_range& range, int& index) {
	int min = range.min;
	int max = range.max;
...
int qh3simple_router::is_port_available(const qstring& host, int port_number) {
	...
	if (getaddrinfo(host.c_str(), port.c_str(), &hints, &local) != 0) {
		DEBUG_PRINT_ERROR(__LOGTAG__, "failed to resolve host. port:%d",
						  port_number);
		return -1;
	}
	...
	if (sock < 0) {
		DEBUG_PRINT_ERROR(__LOGTAG__, "failed to create socket. port:%d",
						  port_number);
		freeaddrinfo(local);
		return -1;
	}
	...
	if (fcntl(sock, F_SETFL, O_NONBLOCK) != 0) {
		DEBUG_PRINT_ERROR(__LOGTAG__, "failed to make socket non-blocking. port:%d",
						  port_number);
		freeaddrinfo(local);
		close(sock);
		return -1;
	...
	if (bind(sock, local->ai_addr, local->ai_addrlen) < 0) {
		DEBUG_PRINT_ERROR(__LOGTAG__, "failed to connect socket. port:%d",
						  port_number);
		freeaddrinfo(local);
		close(sock);
		return -1;
...
void qh3simple_router::cmd_feedback_from_client(struct sockaddr* client_addr,
												const qstring& cmd) {
	char host[INET6_ADDRSTRLEN];
	char port[10];
	int ret =
		getnameinfo(client_addr, sizeof(struct sockaddr), host, sizeof(host),
	...
	// if found delete
	if (found) {
		if (cmd.compare(qstring::format_string("shut-ack-%s", port)) ==
			0) { // shut downed
			size_t oldSz = routes.size();
			routes.erase(std::remove(routes.begin(), routes.end(), found),
						 routes.end());
			if (oldSz != routes.size()) {
				DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__, "Removed route %s:%s from router",
							found->host.c_str(), found->port.c_str());
				GX_DELETE(found);

				// if no routes then shut down command center
				if (routes.size() == 0) {
					conn_io_req_res* req =
						conn_io_req_res::create("/shutdown_cmd_center", "");
					qh3client_helper::send_async_request<client::qh3client>(
						command_route->host, command_route->port, req, nullptr,
						[](conn_io_req_res* response, void* client_specific_data,
		...
		if (command_route->host == host && command_route->port == port) {
			if (cmd.compare(qstring::format_string("shut-ack-%s", port)) ==
				...
				DEBUG_PRINT(LOG_LEVEL_0, __LOGTAG__,
							"Removed command-route %s:%s from router",
							command_route->host.c_str(), command_route->port.c_str());
				GX_DELETE(command_route);
				ev_break(mainloop, EVBREAK_ONE);
			}
		} else {
			DEBUG_PRINT_ERROR(__LOGTAG__, "route not found %s:%s in the list !!!",
							  host, port);
		}
	}
	//

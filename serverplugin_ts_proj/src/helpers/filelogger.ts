...
//q server log
function LOG_FILE_q(native_server: serversdk.qserver_ptr, type: serversdk.elog_type, tag: string, message: string) {
    LOG_FILE_WITH_ROOMID_q(native_server, type, tag, '', '', message);
}
function LOG_FILE_WITH_PID_q(native_server: serversdk.qserver_ptr, type: serversdk.elog_type, tag: string, pid: string, message: string) {
    LOG_FILE_WITH_ROOMID_q(native_server, type, tag, pid, '', message);
}
function LOG_FILE_WITH_ROOMID_q(native_server: serversdk.qserver_ptr, type: serversdk.elog_type, tag: string, pid: string, roomid: string, message: string): void {
    serversdk.sdklib.qserver_logfile(native_server, serversdk.log_lvls.LEVEL_0, type, tag, pid, roomid, message);
}

export function Q_INFO(native_server: serversdk.qserver_ptr, tag: string, message: string) {
    LOG_FILE_q(native_server, serversdk.elog_type.INFO_LOG, tag, message);
}
export function Q_DEBUG(native_server: serversdk.qserver_ptr, tag: string, message: string) {
    LOG_FILE_q(native_server, serversdk.elog_type.DEBUG_LOG, tag, message);
}
export function Q_WARN(native_server: serversdk.qserver_ptr, tag: string, message: string) {
    LOG_FILE_q(native_server, serversdk.elog_type.WARN_LOG, tag, message);
}
export function Q_ERROR(native_server: serversdk.qserver_ptr, tag: string, message: string) {
    LOG_FILE_q(native_server, serversdk.elog_type.ERROR_LOG, tag, message);
}

export function Q_INFO_WITH_PID(native_server: serversdk.qserver_ptr, pid: string, tag: string, message: string) {
    LOG_FILE_WITH_PID_q(native_server, serversdk.elog_type.INFO_LOG, tag, pid, message);
}
export function Q_DEBUG_WITH_PID(native_server: serversdk.qserver_ptr, pid: string, tag: string, message: string) {
    LOG_FILE_WITH_PID_q(native_server, serversdk.elog_type.DEBUG_LOG, tag, pid, message);
}
export function Q_WARN_WITH_PID(native_server: serversdk.qserver_ptr, pid: string, tag: string, message: string) {
    LOG_FILE_WITH_PID_q(native_server, serversdk.elog_type.WARN_LOG, tag, pid, message);
}
export function Q_ERROR_WITH_PID(native_server: serversdk.qserver_ptr, pid: string, tag: string, message: string) {
    LOG_FILE_WITH_PID_q(native_server, serversdk.elog_type.ERROR_LOG, tag, pid, message);
}
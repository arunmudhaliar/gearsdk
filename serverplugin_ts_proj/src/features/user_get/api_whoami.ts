...
class api_whoami implements server.interface_api {
    ...
    private async parse_whoami(native_server: serversdk.qh3server_ptr, cid: Buffer, cid_len: number, user_server_interface: server.interface_userserver, api_instance: server.interface_api, path: string, buffer: string, headers: string): Promise<string | null> {

...
void QNetworkClient::flush_egress(struct ev_loop *loop, struct conn_io *conn_io) {
    ...
    printf("\n\n========= >>>>>> Timer %f %llu\n\n", t, quiche_conn_timeout_as_nanos(conn_io->conn));
...
int QNetworkClient::run(std::string host, std::string port) {
    ...
    quiche_config_set_max_idle_timeout(config, 100000);

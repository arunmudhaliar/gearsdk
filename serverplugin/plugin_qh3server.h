...
#ifndef PLUGIN_QH3SERVER_H
#define PLUGIN_QH3SERVER_H
...
class plugin_qh3router_event_listener : public observer_router_events {
   public:
	~plugin_qh3router_event_listener() {}
	...
	plugin_qh3router_event_listener(type_on_router_pre_start pre_start_cb, type_on_router_start start_cb, type_on_router_stop stop_cb, type_on_router_error error_cb)
...
class plugin_qh3server_event_listener : public observer_qh3server_events {
   public:
	~plugin_qh3server_event_listener() {}
	...
	plugin_qh3server_event_listener(type_on_server_pre_start pre_start_cb, type_on_server_start start_cb, type_on_server_stop stop_cb, type_on_server_error error_cb, type_on_server_parse parse_cb)
...
struct st_plugin_qh3server_response_packet {
    st_plugin_qh3server_response_packet(qh3server* server, uint8_t *cid, uint16_t cid_len, const char* payload, size_t len) {
        this->server = server;
    ...
    ~st_plugin_qh3server_response_packet() {
        if (this->cid) {
            free(this->cid);
            this->cid = nullptr;
        }
...
class plugin_qh3server : public qh3server {
   public:
    ...
    struct st_admin_cmd {
        st_admin_cmd(short type, plugin_qh3server* server): type(type), server(server) {
        }
        short type = 0;
        plugin_qh3server* server = nullptr;
    };
...
//    }
    ...
    async_ev_notifier<struct st_plugin_qh3server_response_packet>& get_ev_notifier()   { return ev_notifier; }
...
private:
    async_ev_notifier<struct st_plugin_qh3server_response_packet> ev_notifier;

...
{
    // Define delegate type for message handlers
    public delegate void MessageCallback(int room, string pid, ulong len, byte[] buf, string msg);
    ...
    // Call a handler based on messageId
    public bool HandleMessage(ulong t_crc, int room, string pid, ulong len, byte[] buf, string msg)
    {
        if (_handlers.TryGetValue(t_crc, out var callback))
        {
            callback(room, pid, len, buf, msg);
            return true;
        }
        else
        {

...
void finish_and_destroy_work(WorkData* data) {
	...
	std::cout << "\r" << COLOR_CYAN << "Workers remaining: " << app_state.workers_map.size() 
...
void gclient_onconnect_cb(gclient* c, conn_io_client* qconnection) {
	debug_print(LOG_LEVEL_2, __LOGTAG__, "gclient %x - Connected", qconnection->cid_hash_val);
	Document doc;
...
void gclient_onmessage_cb(gclient* c, ssize_t recv_len, uint8_t* buf, conn_io_client* qconnection) {
	debug_print(LOG_LEVEL_2, __LOGTAG__, "gclient %x - Received message: %.*s", qconnection->cid_hash_val, recv_len, buf);
}

void gclient_onreleaseconnection_cb(gclient* c, conn_io_client* qconnection) {
	debug_print(LOG_LEVEL_2, __LOGTAG__, "gclient %x - Releasing connection", qconnection->cid_hash_val);
	finish_and_destroy_work(static_cast<WorkData*>(c->get_user_data()));
}

void gclient_onclose_cb(gclient* c, conn_io_client* qconnection) {
	debug_print(LOG_LEVEL_2, __LOGTAG__, "gclient %x - Connection closed", qconnection->cid_hash_val);
}
...
void exit_after_delay(uv_timer_t* handle) {
	...
	if (app_state.workers_map.size() > 0) {
		std::cout << "Waiting for pending requests to finish... pending: " << app_state.workers_map.size() << std::endl;

...
void gclient_onconnect_cb(gclient* c, conn_io_client* qconnection) {
	...
	if (!app_state.single_request) {
		...
		GX_DELETE(msg);
	} else if (app_state.requests.size()) {
		const auto& request = app_state.requests[app_state.single_request_index % app_state.requests.size()];
		if (request.first.compare("onconnect-send") == 0) {
			debug_print(LOG_LEVEL_0, __LOGTAG__, "send --> %s", request.second.c_str());
			c->send_message(request.second, true);
		} else if (request.first.compare("onconnect-close") == 0) {
			c->close();
		} else {
			debug_print_error(__LOGTAG__, "Invalid request command. Returning ...");
		}
	} else {
		debug_print_error(__LOGTAG__, "Invalid request. Returning ...");
	}
}
